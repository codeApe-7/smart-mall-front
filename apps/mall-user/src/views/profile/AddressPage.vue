...
async function submitAddress() {
  ...
  await saveAddress(sessionStore.userToken, form);
...
async function removeAddress(id?: string) {
  if (!sessionStore.userToken || !id) {
    return;
  }
  await deleteAddress(sessionStore.userToken, id);
  await loadAddresses();
}
...
async function markDefault(id?: string) {
  ...
  await setDefaultAddress(sessionStore.userToken, id);
  await loadAddresses();
}
    ...
    <section class="address-page__grid">
      <article v-for="item in addresses" :key="item.addressId" class="mall-panel address-page__card">
        <div class="section-title">
          ...
          <el-tag v-if="item.defaultAddress === 1" type="success">默认</el-tag>
        </div>
        <p>{{ item.receiverPhone }}</p>
        <p class="dimmed">{{ item.fullAddress || item.detailAddress }}</p>
        <div class="address-page__actions">
          <el-button link @click="editAddress(item)">编辑</el-button>
          <el-button link @click="markDefault(item.addressId)">设为默认</el-button>
          <el-button link type="danger" @click="removeAddress(item.addressId)">删除</el-button>
        </div>
      </article>
    </section>
    <el-dialog v-model="dialogVisible" title="编辑地址" width="640px">
      <el-form label-position="top">
        ...
        <el-form-item label="省市区">
          ...
          <el-input v-model="form.city" placeholder="市" style="margin-top: 10px" />
          <el-input v-model="form.region" placeholder="区" style="margin-top: 10px" />
        </el-form-item>
