...
export const chatAssistantAgent = (data: { userId: string; sessionId?: string; message: string; productId?: string; orderId?: string; refundReason?: string }) => request.post<AssistantChatResponse>('/assistant/agent/chat', data);
export const getAssistantHistory = (data: { userId: string; sessionId?: string; pageNo?: number; pageSize?: number }) => request.post<PageResult<AssistantHistoryItem>>('/assistant/history', data);
