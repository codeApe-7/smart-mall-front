...
async function loadProfile() {
  ...
  const profile = await getProfile(sessionStore.userToken);
...
async function saveCurrentProfile() {
  ...
  await saveProfile(sessionStore.userToken, profileForm);
...
async function savePassword() {
  ...
  await updatePassword(sessionStore.userToken, passwordForm);
  ElMessage.success('密码更新成功');
}

async function doLogout() {
  if (sessionStore.userToken) {
    await logout(sessionStore.userToken).catch(() => undefined);
  }
